...
The infrastructure layer lives in `infra/` and manages the AKS cluster, resource group, and Azure RBAC. The stack layer lives in `software/stack/` and manages all Kubernetes workloads, including both platform middleware (Elasticsearch, PostgreSQL, Redis, RabbitMQ, MinIO, Keycloak, Airflow) and OSDU services (Partition, Entitlements, and future services). Combining middleware and services in a single state is intentional: they share the same change frequency and deployment lifecycle, and OSDU services have `depends_on` relationships with middleware modules.
